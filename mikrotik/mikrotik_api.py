...
def testConvert():
    data = ['re', '.id=*1', 'interface=wlan1', 'mac-address=B8:27:EB:0C:F8:3D', 'ap=false', 'wds=false', 'bridge=false',
     'rx-rate=72.2Mbps-20MHz/1S/SGI', 'tx-rate=36Mbps', 'packets=1123,433', 'bytes=44025,59534', 'frames=1123,505',
     'frame-bytes=52159,64639', 'hw-frames=14108,37457', 'hw-frame-bytes=1006095,971655', 'tx-frames-timed-out=0',
     ...
     'authentication-type=wpa2-psk', 'encryption=aes-ccm', 'group-encryption=aes-ccm', 'management-protection=false',
...
def getRegisteredDevices(host, username, password):
    ...
    response = dict(item.split("=") for item in [item[1:] for item in response] if "=" in item)
    # print(response)
    print(response['comment'], response['mac-address'], response['uptime'])
